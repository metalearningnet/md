...
class MD(nn.Module):
    def __init__(self,
        ...
        info(f"MD (strategy: {config.strategy_info}, preference: {config.po}, special_tokens: {self.num_special_tokens}, hidden_dim: {self.hidden_size})")
    ...
    def _backend_fuse_features(self, state_embeds, action_embed, attention_mask=None):
        ...
        last_valid_states_dtype = last_valid_states.to(self.dtype)
        action_embed_dtype = action_embed.to(self.dtype)
        
        state_norm = self.backend_state_norm(last_valid_states_dtype.float()).to(self.dtype)
        action_norm = self.backend_action_norm(action_embed_dtype.float()).to(self.dtype)
        ...
        if attention_mask is not None:
            updated_state_embeds = updated_state_embeds.clone()
    ...
    def forward(
        ...
        if self.backend_anno:
            ...
            if self.backend_enable_fusion:
                ...
                action_embed = self.backend_action_proj(last_valid_actions)
                state_embeds = self._backend_fuse_features(state_embeds, action_embed, attention_mask)
            ...
            for idx in active_indices:
                ...
                if self.backend_enable_fusion:
                    with torch.no_grad():
                        ...
                        action_logits = skill_output['action_logits']
                        last_valid_actions = self._get_last_valid_actions(action_logits)
                        action_embed = self.backend_action_proj(last_valid_actions)
                        context_embeds = self._backend_fuse_features(context_embeds, action_embed)
